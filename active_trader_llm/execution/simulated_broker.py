"""
Simulated Broker: Pure simulation for backtesting without external APIs.
...
class SimulatedBroker:
    """
    Pure simulation broker for backtesting.
    ...
    def update_cash(self, cash: float, equity: float):
        """
        Update cash and equity from external source (e.g., BacktestEngine).
    ...
    def submit_trade(
        ...
        try:
            ...
            # Deduct from cash (will be updated by BacktestEngine on position open)
            # We don't actually modify self.cash here to avoid double-counting
            # BacktestEngine will handle cash updates when position is opened
