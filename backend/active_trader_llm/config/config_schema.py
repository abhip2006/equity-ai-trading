...
class LLMConfig(BaseModel):
    provider: Literal["openai", "anthropic", "google", "xai", "deepseek", "openrouter", "local"] = "openai"
    model: str = "gpt-4.1-nano"
    api_key: Optional[str] = None
    base_url: Optional[str] = None  # For OpenAI-compatible APIs (e.g., DeepSeek)
...
class ExecutionConfig(BaseModel):
    ...
    """
    broker: Literal["alpaca"] = "alpaca"  # FORCED: Only Alpaca broker allowed
...
class Config(BaseModel):
    ...
    mode: Literal["paper-live", "live"] = "paper-live"
